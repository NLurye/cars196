#this is an experiment to see whether the augmentation is improving the accuracy or not
...
cars_test, cars_val, cars_train = tfds.load('Cars196',data_dir='C:/Users/anast/PycharmProjects/cars196/data', as_supervised=False, shuffle_files=True, split=["test", "train[0%:20%]", "train[20%:]"])
...
cars_train_pp_aug = cars_train_pp.map(lambda x, y: (data_augmentation(x, training=True), y), num_parallel_calls=tf.data.AUTOTUNE)
...
image_batch, label_batch = next(iter(cars_train_pp_aug.take(1)))
random_index = tf.random.uniform(shape=[], minval=0, maxval=image_batch.shape[0], dtype=tf.int32)
random_image = image_batch[random_index]
random_label = label_batch[random_index]
class_index = tf.argmax(random_label)
...
model_3 = Model(inputs=base_model.input, outputs=prediction_layer)
model_3.summary()
...
model_3.compile(optimizer=opt, loss=loss_fn, metrics=['accuracy'])
hist = model_3.fit(cars_train_pp_aug, epochs=epochs, validation_data=cars_val_pp, workers=n_workers)
...
y_pred = model_3.predict(cars_test_pp)
